...
# attachment(list): 邮件的附件, send(str): 发送人, file_code(str):创建文件夹的前缀，通过邮寄id和编号组成
def filter_message(attachment, send: str, file_code: str):
    date_time = datetime.datetime.now().strftime("%Y-%m-%d")
    if send == "OFR":  # 发件人分类 目前只有一个
        file_path = pdf_storage_location + "\\" + send + "\\" + date_time + "\\" + file_code  # 存放附件的文件名
        if not os.path.exists(file_path):  # 创建文件 没有则创建，可以多层创建
        ...
        for a in attachment:  # <class "imap_tools.message.MailAttachment">
            ...
            if length == 0 and judge_digit is True:  # 优先写入Clearance，其次Duty 判断循环是否到达最后，且是否遍历过Duty附件，都满足执行流程
...
# string_value(str)：邮件的subject, uid(str): 邮件的uid
# 过滤邮件subject的编码
...
# files_path(list): pdf文件的路径
# 调用java接口转化pdf为base64
...
# pdf_json(json): pdf的json数据, bill_json(json): 账单的json数据， filename(str): pdf文件的路径
# 提取pdf，bill的数据，sea_amount最后单独写入excel
def parse_pdf_bill(pdf_json, bill_json, filename):
    ...
    pdf_list_all = []
    # eya货柜单清关预览 判断key是否存在字典里面，如果存在存入数据，不存在返回''
    pdf_list = []
...
# start_time(str): 查询邮件大于该时间之后的邮件，默认今天
def start(start_time=""):
    if start_time == "":
        start_time = datetime.datetime.now().strftime("%Y-%m-%d")
    mail_pass = fs_mail_pass
    with MailBox(fs_host).login(fs_username, mail_pass, initial_folder="INBOX") as mailbox:  # 登录邮件
        print('登录邮箱成功')
        a = str(start_time).split('-')
        criteria = AND(date_gte=datetime.date(int(a[0]), int(a[1]), int(a[2])))  # 邮件的过滤条件
        ...
        for msg in mailbox.fetch(criteria, charset="utf-8"):
            ...
            if exist:
                continue
            subject = msg.subject  # 邮件主题
            attachments = msg.attachments  # 邮件附件
            if fs_receive_email in sender:  # test
                count += 1
                uid_subject[uid] = subject
                if ("OFR" in subject) and ("HBL" in subject) and ("NLRTM" in subject):  # 筛选邮件
...
# uid_list(dict): 邮件id的集合, date_time(str): 筛选的时间默认为空
def parse_pdf_bill_findings(uid_list, date_time=""):
    uid_all = []
    for uid_item in uid_list:  # 拿到dict所有的values值
    ...
    file_list_list = []
    if not len(list(file_list.keys())):  # 过滤掉不在uid_list里面的pdf
        return [], []
    ...
    for key in file_list.keys():  # 解析pdf附件
...
# uid_list(dict):所有邮件的uid, problem_uid(dict)：错误邮件的uid
# 过滤uid_list里面的problem_uid 剩下的就是正确邮件的uid
...
# uid_list(dict)：正确的uid  problem_uid(dict): 有问题的uid, bedrag_list_all(list)：正确的uid的关税, uid_subject(dict)：邮件subject的集合
# 发送邮件
def send_email_by_judge(uid_list, problem_uid, bedrag_list_all, uid_subject):
    ...
    for left in problem_uid:
        ...
        try:
            ...
            attach_path = excel_fullpath_all[uid]  # 找到excel路径放到邮件附件发送
            # print(attach_path)
...
# sending_succeeded_uid(str):正确邮件的uid, problem_uid(str)：错误邮件uid
# 写入redis作唯一判断
...
# validity(dict): 正确的excel, invalidity(dict)： 错误的excel
# 对正确错误的excel1进行分类，放入到不同的文件夹
def classify_files(validity, invalidity):
    ...
    validity_email = []
    invalidity_email = []
    for path, dir_list, file_list in g:  # 分类正确错误的excel的id
    ...
    for dir1 in os.listdir(excel_path):  # file folder  all will be printed out
        ...
        if cur_path in validity_email:
            new_file_path_previous = os.path.join(excel_path, 'validity')  # 判断是否是正确的文件，
            new_file_path = os.path.join(new_file_path_previous, dir1)  # 创建路径
...
# time_date(str)：默认今天, uid_list(list):正确uid  problem(list):错误的uid
# pdf的绝对路径
def absolute_path_pdf(time_date="", uid_list=[], problem_uid=[]):  # return file of pdf
    ...
    a = traverse_folder(time_date)  # 拿到文件所有的pdf信息
    a_path_list = []
    if len(uid_list) != 0 or len(problem_uid) != 0:  # 过滤pdf
    ...
    for i in a.keys():  # 处理pdf信息
        ...
        item = [file_path, file_name] # pdf 的文件夹和文件名
